...
def _analyze_single_campaign_conflict(
    ...
    if chunk_size is None:
        chunk_size = config.CONFLICT_CHUNK_SIZE
    if model is None:
        model = config.CONFLICT_MODEL

    total_turns = len(df)
    window_size = config.CONFLICT_EXTRACTION_WINDOW
    overlap = config.CONFLICT_EXTRACTION_OVERLAP
...
def analyze_conflict(
    ...
    Args:
        ...
        chunk_size: Turns per chunk (default: config.CONFLICT_CHUNK_SIZE)
        model: LLM model (default: config.CONFLICT_MODEL)
